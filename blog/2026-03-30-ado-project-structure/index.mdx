...
authors:
  - vanescoboehm
tags: [Azure DevOps, Project Structure, Agile, Area Paths, Iterations, Team Configuration]
...
The default team subscribes to the root area path and selects sprints as they become active. The backlog shows all work items; the sprint board shows items assigned to the current iteration. No further configuration is needed.
...
├── Fabrikam\Team Alpha            ← Team Alpha (subscribes to this node only)
├── Fabrikam\Team Beta             ← Team Beta (subscribes to this node only)
└── Fabrikam\Team Gamma            ← Team Gamma (subscribes to this node only)
...
Delivery teams subscribe exclusively to their own leaf-node area path. The management or portfolio team subscribes to the root `Fabrikam` with **"Include sub-areas"** enabled, giving it rollup visibility across all delivery teams without claiming ownership of their boards.
...
- **Area path subscription**: all product area paths (so the team's backlog shows work across all products, filterable by area path)
...
**Overlapping area path subscriptions on Kanban boards.** When two teams subscribe to the same area path with "Include sub-areas" enabled, the `Board Column` and `Board Lane` fields on shared work items reflect whichever team last moved them. Keep area path subscriptions exclusive at the leaf level for any team that uses a Kanban board.

**Not setting dates on iteration paths.** Burndown charts, Delivery Plans, and the `@CurrentIteration` query macro all depend on start and finish dates being set on iteration nodes. Creating sprints without dates silently breaks these features from day one.

**Using tags as primary categorization.** Tags are convenient but ungoverned — there is no approval workflow, no hierarchy, and no way to delete them through the UI. Use area paths for product ownership and custom picklist fields for structured reporting attributes. Reserve tags for lightweight, informal labels like "blocked" or "customer-reported."
...
**Creating subiterations but not updating team iteration settings.** When you add a new subiteration, each team must go to *Project Settings → [Team] → Iterations* and mark it as active. This is the most common operational mistake with the subiteration pattern and easy to overlook during sprint kickoff.
...
The architectural principle behind all three scenarios is the same: **teams are views, not containers.** A team is a configured lens that filters the project's shared work item pool through area path subscriptions and iteration selections. Understanding this makes every configuration decision a deliberate choice about what each team needs to see — rather than a structural wall between teams.
...
- [**Define area paths and assign to a team** — Step-by-step guide for configuring the area path hierarchy and subscribing teams to the right nodes.](https://learn.microsoft.com/en-us/azure/devops/organizations/settings/set-area-paths)
