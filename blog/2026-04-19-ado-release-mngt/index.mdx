...
The cost is visibility. Links between a Story and a Release WIT do not surface as a filter option in the sprint board or sprint backlog. Sprint board filters operate on iteration path and area path — not on linked work item types. This is an architectural limitation of the product, not a configuration gap. Seeing release membership in sprint context requires a saved query ("all items in Sprint 22 with a Related link to Release 2.5"), not a toggle in the board UI.

Setup requires process customization at the organization level — creating an inherited process, adding the type and its fields, and migrating the project to the new process. These are one-time tasks, but they need a process administrator and affect every project on that process template.
...
There are also practical limits worth knowing: a single work item can carry at most 100 tags, only the first 200 tags in the project appear in autocomplete, and performance degrades in collections with more than 100,000 tags.
...
The field can be used as a filter criterion in Delivery Plans via its field criteria settings, and works natively with Analytics, Power BI, and query-based charts. Grouping by a clean categorical field requires no text-splitting workarounds. The standard backlog filter toolbar does not expose custom fields — filtering by the field in backlog context requires a saved query.

One important limitation: the field is not available as a filter in the sprint board or sprint backlog view. Sprint board filters operate on iteration path and area path — custom fields are not exposed there. This is a product constraint, not a misconfiguration. Seeing release membership during sprint execution requires a saved query rather than a UI toggle, which is a genuine daily-use gap that teams should factor in before choosing this option.
...
| **Governed values** | Name only (+/-) | Custom fields (+) | Free-form (-) | Enumerated picklist (+) | Enumerated via extension (+) | Defined by admin (+) |
| **Backlog / Delivery Plans filter** | Via parent hierarchy (+) | Requires saved query (-) | Native (+) | Delivery Plans only (+/-) | Delivery Plans only, "Contains" (+/-) | Native (+) |
| **Sprint board visibility** | "Show parents" only (+/-) | Not available (-) | Flattens hierarchy (+/-) | Not available (-) | Not available (-) | Native (+) |
| **Release has own metadata** | Dates, owner, description (+) | Full custom form (+) | String only (-) | String only (-) | String only (-) | Dates & name only (+/-) |
| **Release has own workflow** | Inherits WIT states (+/-) | Custom states (+) | (-) | (-) | (-) | (-) |
| **Analytics / Power BI** | Hierarchy rollup (+) | Via linked-item queries (+/-) | No chart grouping (+/-) | Clean categorical field (+) | Needs text-split logic (+/-) | Works, conflates time & scope (+) |
| **Setup complexity** | None | Medium | None | Low | Low & extension install | None |
| **Extension required** | No | No | No | No | Yes (+/-) | No |
| **Conflicts with standard usage** | If Features already in use (+/-) | No conflict (+) | No conflict (+) | No conflict (+) | No conflict (+) | Conflates time & scope (+/-) |

---

## Decision Guide

**The team needs a quick release label, is small, and can enforce a naming convention by discipline.**
→ Tags. Zero configuration, multi-release assignment. Accept the governance risk consciously.

**Single-value release assignment covers most cases; reporting and analytics on release scope matter.**
→ Custom picklist field. Clean categorical data, governed enumeration, works natively with Analytics and Power BI. Use saved queries to surface release membership in sprint context.

**Stories genuinely span two releases and governance matters.**
→ Multi-value picklist extension if a Marketplace extension can be approved. If not, Tags with a strict naming convention as a conscious fallback.

**The team needs a Release entity with its own lifecycle — target date, owner, release notes, workflow states.**
→ Custom WIT. Accept that release membership is surfaced via queries rather than inline board filters.

**Features are already used for product capabilities and cannot be repurposed.**
→ Custom WIT or custom picklist field, depending on whether release metadata and multi-release assignment are required.

**Features are free to use and the team wants cross-team release planning in Delivery Plans.**
→ Feature/Epic as container. Pairs naturally with Delivery Plans for timeline visualization with no additional configuration.

**Audit-quality traceability is required — what shipped in each build, deployed when, to which environment.**
→ Pipeline-driven links, layered on top of whichever planning option is in use.
...
- [**Define features and epics to organize backlog items** — Microsoft Learn](https://learn.microsoft.com/en-us/azure/devops/boards/backlogs/define-features-epics?view=azure-devops)
...
- [**Work-items tracking using Release Management** — Azure DevOps Blog post covering pipeline-driven traceability in depth.](https://devblogs.microsoft.com/devops/work-items-tracking-using-release-management/)
